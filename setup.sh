...
if [ "$ENVIRONMENT" = "dev" ]; then
    docker-compose -f docker-compose.yml -f docker-compose.dev.yml exec -T web python manage.py makemigrations
...
if [ "$ENVIRONMENT" = "dev" ]; then
    docker-compose -f docker-compose.yml -f docker-compose.dev.yml exec -T web python manage.py makemessages -l pt_BR -l en --ignore=.venv
...
if [ "$RUN_TESTS" = true ] && [ "$ENVIRONMENT" = "dev" ]; then
    ...
    if [ "$RUN_COVERAGE" = true ]; then
        docker-compose -f docker-compose.yml -f docker-compose.dev.yml exec -T web coverage run manage.py test core
        ...
        docker-compose -f docker-compose.yml -f docker-compose.dev.yml exec -T web python manage.py test core
        print_success "Tests completed"
    fi
else
    if [ "$ENVIRONMENT" = "prod" ]; then
